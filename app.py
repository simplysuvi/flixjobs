...
jobs_df = jobs_df.sort_values(by='Posting Date', ascending=True)
...
with st.expander('**Recently :green[Added] Jobs**'):
    if not added_jobs.empty:
        st.dataframe(added_jobs[['Id','Title', 'Posting Date', 'Location']], use_container_width=True, hide_index=True)
    else:
